...
Unique representation? Counter example (bivariate normal?)
Equivalence class up to equal likelihood


Q1. Yes. By consistency of GLM.

Given the graph structure, 
f(x_1, x_2, ..., x_p) = f

## General ideas
Consider IID $\vec{x_i}, i = 1, 2, ..., n$

KL divergence:
loglikelihood of the true model: $\int_{-\infty}^{\infty} f(x) \log f(x) dx$
loglikelihood of the working model: $\int_{-\infty}^{\infty} f(x) \log g(x) dx$
Deviance: 
$$\displaystyle \int_{-\infty}^{\infty} f(x) \left[\log f(x) - \log g(x)\right] dx 
= \int_{-\infty}^{\infty} f(x) \log \dfrac{f(x)}{g(x)} dx = E\left[\log{\dfrac{f(x)}{g(x)}}\right]$$

Deviance:
$$\displaystyle \sum_{i=1}^n \left[\log f(\vec{x_i}) - \log g(\vec{x_i})\right]
= \sum_{i=1}^n \log \dfrac{f(\vec{x_i})}{g(\vec{x_i})}$$

\begin{align}
\prod_{i=1}^n f(\vec{x_i}) 
\\= \prod_{i=1}^n \prod_{j=1}^{p} f_j(x_j | x_{-j})
\\= \prod
